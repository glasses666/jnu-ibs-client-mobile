...
interface RoomBindingProps {
  userId: string;
  onBindSuccess: (roomId: string, nickname: string) => void;
}
...
export const RoomBinding: React.FC<RoomBindingProps> = ({ userId, onBindSuccess }) => {
  ...
  const handleBind = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onBindSuccess(roomId.toUpperCase(), nickname.trim() || 'User');
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="fixed inset-0 bg-white dark:bg-gray-900 flex flex-col items-center justify-center p-6 animate-fade-in z-50">
      <div className="w-full max-w-sm text-center">
        ...
        <p className="text-gray-500 mb-8">请填写您的信息以同步数据</p>
        ...
        <form onSubmit={handleBind} className="space-y-4">
          <div className="space-y-4">
            <input 
                ...
                placeholder="您的昵称 (例如: 宿舍长)"
                className="w-full px-6 py-4 bg-gray-50 dark:bg-gray-800 rounded-2xl border-2 border-transparent focus:border-indigo-500 focus:bg-white dark:focus:bg-gray-800 outline-none font-bold text-center text-lg placeholder:font-medium transition-all"
                autoFocus
            />
            <input 
                type="text" 
